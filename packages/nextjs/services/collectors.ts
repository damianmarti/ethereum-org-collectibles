...
export async function fetchCollectors(year: string) {
  ...
  for (const collectible of collectibles) {
    ...
    try {
      if (collectible.source === "POAP") {
        collectors = await fetchPoapCollectors(collectible.id, process.env.POAP_API_KEY);
      } else if (collectible.source === "GitPOAP") {
        if (!collectible.poap_event_id) continue;
        collectors = await fetchPoapCollectors(collectible.poap_event_id, process.env.POAP_API_KEY);
